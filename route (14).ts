...
const schema = z.object({ date:z.string().regex(/^\d{4}-\d{2}-\d{2}$/), personId:z.string().uuid(), label:z.string().max(180).nullable().optional(), categoryId:z.string().uuid(), amount:z.number().positive(), currency:z.string().length(3), note:z.string().max(2000).nullable().optional() });
  ...
  await prisma.$transaction(async tx => {
    if (d.label?.trim()) {
      ...
      await tx.reusableItem.upsert({
        ...
        update: { name: label, categoryId: category.id, usageCount: { increment: 1 }, lastUsedAt: new Date() },
        create: { userId: user.id, kind: "INCOME", name: label, normalized, categoryId: category.id, usageCount: 1, lastUsedAt: new Date() },
      });
    }
    await tx.incomeEntry.create({ data: {
      userId:user.id, personId:d.personId, categoryId:category.id, date:txDate, label, amount:d.amount, currency:d.currency.toUpperCase(), note:d.note?.trim()||null,
      fxRate:fx?.rate??null, fxDate:fx?.date??null, fxSource:fx?.source??"pending", baseAmount:fx?.baseAmount??null,
    }});
  });
  return Response.json({ ok: true, fxStatus: fx ? "ready" : "pending" });
}
